...
static int push_repeated_msg(lua_State* L, Message* msg, FieldDescriptor* field)
{
...
{
    lua_repeated_msg* repeated = (lua_repeated_msg*)luaL_checkudata(L, 1, PB_REPEATED_MESSAGE_META);
...
static int pb_new(lua_State* L)
{
    ...
    Message* message = GetProto().CreateMessage(type_name);
    if (!message)
    {
